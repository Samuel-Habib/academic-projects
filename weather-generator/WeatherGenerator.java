/*
 *
 * @author: Samuel Habib, smh389, 217007500
 * 
 ...
 */
 ...
 public class WeatherGenerator {
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        double firstRandom = StdRandom.uniformDouble();
        if (firstRandom >= 0.5) {
            arr[0] = DRY;
        }
        else {
        ...
        for (int i=1;i<numberOfDays;i++) {
            double secondRandom = StdRandom.uniformDouble();
            if (arr [i-1] == WET) {
                if (secondRandom <= wetwetProbability) {
                    arr[i] = WET;
                }
                else {
                    arr[i] = DRY;
                ...
                if (secondRandom <= drywetProbability) {
                    arr[i] = WET;
                }
                else {
                    arr[i] = DRY;
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude ){
        ...
        populateArrays(drywet,wetwet);
        double[] drywet_prob = new double[12];
        double[] wetwet_prob = new double[12];
        populateLocationProbabilities(drywet_prob, wetwet_prob, longitude, latitude, drywet, wetwet);
        int[] forecasted_weather = forecastGenerator(drywet_prob[month], wetwet_prob[month], numberOfDaysInMonth[month]);
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        for (int i=0;i<forecast.length-6;i++) {
            sum = forecast[i] + forecast[i+1] + forecast[i+2]+forecast[i+3] +forecast[i+4] +forecast[i+5]+ forecast[i+6];
            if (sum >max) {
                max = sum;
                week=i;
            }
